---
PLAN: "refactor!: indexdb implementa db.Conn (contrato movido de orm a tinywasm/db) — solo DML"
TAG: v0.4.0
---

# PLAN — `tinywasm/indexdb`: migrar de `*orm.DB` a `db.Conn`
...
> Este plan **requiere `tinywasm/db@v0.0.1` ya publicado**. Si no resuelve en `go get`, para y
...
es — se extrajo a `tinywasm/db`. Ahora:

- `adapter` implementa **`db.Conn`** (Executor+Compiler unidos) en vez de `orm.Executor`+`orm.Compiler`
  vía `orm.New(adapter, compiler)`.
- `New(dbName, idg, logger, structTables...)` devuelve **`db.Conn`**, no `*orm.DB`.
- Se prueba contra **`db/conformance`** (no `orm/conformance`).
- `go.mod` final: `db`+`model`+`fmt`+`jsvalue`. **Cero `tinywasm/orm`.**
...
El puerto `tinywasm/db` reemplaza el contrato que antes vivía en `orm`. `indexdb` entra **solo** en su
mitad DML (`db/conformance`) — su esquema no es DDL SQL: los object stores se declaran por adelantado
...
- `tests/setup_test.go` ya define `idGenerator` (counter) y `SetupDB(logger func(...any), dbName
  string, structTables ...any) *orm.DB` — se adapta a devolver `db.Conn` (§3.3).
...
### 3.2 `adapter.go` — `db.Conn`, sin cambios de lógica interna
...
func (d *adapter) QueryRow(query string, args ...any) db.Scanner       { /* sin cambios */ }
func (d *adapter) Query(query string, args ...any) (db.Rows, error)    { /* sin cambios */ }
func (d *adapter) Close() error                                        { /* sin cambios */ }

// simpleScanner/simpleRows: mismo cuerpo, implementan db.Scanner/db.Rows en vez de orm.*.

type compiler struct{}

func (c *compiler) Compile(q db.Query, m Model) (db.Plan, error) { /* sin cambios de cuerpo */ }

// adapter debe implementar Compile también, delegando al compiler embebido — db.Conn exige
...
func (d *adapter) Compile(q db.Query, m Model) (db.Plan, error) {
	return d.compiler.Compile(q, m)
}

// New builds a fresh IndexedDB-backed db.Conn. dbName must be unique per independent database
// (tests use one dbName per conformance clause for isolation — see setup_test.go, §3.3).
func New(dbName string, idg idGenerator, logger func(...any), structTables ...any) db.Conn {
...
> dentro de `adapter` porque `db.Conn` necesita un solo valor que sea ambas cosas). No inventes un
...
```go
func (d *adapter) execute(q db.Query, m Model, factory func() Model, each func(Model), eachJS func(js.Value)) error { /* sin cambios */ }
func (d *adapter) create(q db.Query, m Model) error   { /* sin cambios */ }
func (d *adapter) update(q db.Query, m Model) error   { /* sin cambios */ }
func (d *adapter) delete(q db.Query, m Model) error   { /* sin cambios */ }
func (d *adapter) readOne(q db.Query, m Model) error  { /* sin cambios */ }
func (d *adapter) readAll(q db.Query, factory func() Model, each func(Model), eachJS func(js.Value)) error { /* sin cambios */ }
func checkCondition(val js.Value, cond db.Condition) bool { /* sin cambios */ }
```

`db.Action`/`db.Query`/`db.Condition` tienen exactamente los mismos campos/getters que sus
...
### 3.4 `tests/setup_test.go` — `SetupDB` devuelve `db.Conn`

```go
func SetupDB(logger func(...any), dbName string, structTables ...any) db.Conn {
...
- **Sync sobre async**: `db.Executor` es **síncrono**; el adapter debe **bloquear** sobre los
...
- **`ReadOne` sin match ⇒ `db.ErrNoRows`** (antes `orm.ErrNoRows`): el scanner debe devolver ese
  sentinela nuevo.
- **Tipos**: `qty int64` y `active bool` round-trip correcto a/desde valores JS.

## 5. Criterios de aceptación

- `adapter` implementa `db.Conn` (`var _ db.Conn = (*adapter)(nil)`). **Cero**
  `github.com/tinywasm/orm` (`grep -rn "tinywasm/orm" .` vacío).
- `New(dbName, idg, logger, structTables...) db.Conn` (antes `*orm.DB`).
...
- `go.mod` en `db@v0.0.1`+; `go mod tidy` limpio; publicado con `gopush`.

## 6. Etapas

| # | Etapa | Archivo(s) | Criterio |
|---|---|---|---|
| 1 | Bump db, quitar orm | `go.mod` | `db@v0.0.1`+ con `db/conformance`; `orm` fuera |
| 2 | `adapter` como `db.Conn` | `adapter.go` | `var _ db.Conn`, `New(...) db.Conn` (§3.2) |
| 3 | Ejecución (solo tipos) | `execute.go` | `db.Query`/`db.Condition` (§3.3) |
| 4 | Test helpers | `tests/setup_test.go` | `SetupDB(...) db.Conn` (§3.4) |
...
| 7 | Publicar | — | `gotest` WASM verde; `gopush 'refactor!: db.Conn'` |
